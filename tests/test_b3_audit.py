...
* **B3.1 -- dispersion df at dmc.py:712**: audit was almost certainly WRONG.
  ...
  chromosome-pooled phi should sit at ~= 1.0 +/- noise. If df were off by a
  factor, phi would be systematically biased.

* **B3.2 -- logit-t Welford M2 at dmc.py:864-868**: audit's specific
  failure mode (jac = 1e12) was hyperbolic since
  ``_logit_variance_jacobian`` clips beta to [1e-6, 1-1e-6]. But the
  *direction* of the concern is valid: at boundary beta values, the delta-
  method Jacobian is huge and the SE inflates. We test directly: does
  logit_t still detect a strong (Deltabeta ~= 0.5) signal when one group sits near
  beta = 0?
...
def synth_md_null(tmp_path_factory):
    ...
    Under H0 the chromosome-pooled Pearson phi should be ~= 1.0 (the
...
def test_b3_1_dispersion_phi_hat_is_unbiased_under_h0(synth_md_null, caplog):
    """B3.1: Under H0 with chrom-pooled dispersion, phi should be ~= 1.0.

    If ``df = n_obs - 2 * n_disp`` is correct (the formula the audit
    questioned), phi tracks ``E[chi^2] / df = 1.0`` under H0. If df were
    off by even a factor of 2, phi would land near 0.5 or 2.0 and this
    test would loudly fail.

    We use dispersion='chrom' to force a single per-chromosome phi that
    ...
    # "%s: chrom-pooled phi = %.3f (raw %.3f, ...)"
    import re
    phi_hats = []
    for rec in caplog.records:
        m = re.search(r"chrom-pooled phi = ([\d.]+)", rec.getMessage())
        if m:
            phi_hats.append(float(m.group(1)))

    assert len(phi_hats) >= 1, (
        "expected at least one chrom-pooled phi log line; got none. "
        "Test cannot verify dispersion-df calibration without it."
    )

    mean_phi = float(np.mean(phi_hats))
    # Under correct df + H0, E[phi] = 1.0. Allow +/-20% wobble -- at 4 vs 4
    # replicates x ~2000 sites the estimator has finite-sample variance
    ...
    assert 0.80 <= mean_phi <= 1.20, (
        f"chrom-pooled phi = {mean_phi:.3f} under H0; expected ~= 1.0. "
...
def test_b3_1_per_site_phi_floors_at_min_dispersion(synth_md_null):
    """B3.1 corollary: under H0, per-site phi_i averages near 1.0 too.

    The 'site' dispersion mode computes per-site phi_i = chi_i / df_i.
    Under H0 with df_i = (n_case + n_ctrl) - 2 = 4 + 4 - 2 = 6, the
    expected chi-square contribution is df_i. So phi_i should average 1.0.
    ...
    # distribution is reasonable. Under H0 with correct df, the chi^2
...
# B3.2 -- logit-t variance at boundary beta
#
...
#   it is well-calibrated near beta = 0 / 1. Two tests that previously
#   locked in an over-aggressive "NaN if either group has M2 = 0" guard
#   were removed: one asserted >=30% NaN at extreme beta (necessarily true
#   only with the strict guard), the other asserted ~5% H0 FP rate at
#   beta ~= 0.005 (only the strict guard could deliver this).
#
#   The strict guard collapsed power to 0 on the standard accuracy
#   fixture (Deltabeta=0.40, cov=20, n=4, replicate_sd=0.03) because ~half the
#   truly-differential hypo sites have beta_treatment clipped to ~0.01 and
...
#   cost of an anti-conservative SE under H0 boundary beta. We accept that
...
#
# The moderate-boundary test below remains: at beta ~= 0.05 the Jacobian
...
def test_b3_2_logit_t_detects_signal_at_moderate_boundary_beta(tmp_path):
    """B3.2 follow-up: at *moderate* boundary beta (~= 0.05, not 0.005) the
    Jacobian inflation is much milder (jac^2 ~= 440 vs 40,000) and almost
    ...
    #
    #   t-stat ~= |logit(0.50) - logit(0.05)| / sqrt(SE_case^2 + SE_ctrl^2)
    ...
    assert frac_strong >= 0.80, (
        f"At moderate boundary beta=0.05 with Deltabeta=0.45 signal, only "
...
# FP rate at beta ~= 0.005) was removed alongside the strict B3.2 guard.
# Under the relaxed guard, logit_t at extreme boundary beta is genuinely
